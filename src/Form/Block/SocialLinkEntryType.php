...
// One entry of SocialLinksType::$links - plain array (no entity, stored straight into the parent Block's JSON "data" column). "network" drives label/icon at render time (see SocialLinkExtension and SocialLinks.html.twig), so they're never stored here - leaving it empty is the escape hatch for a network with no icon in public/icons/ (or bundles/*/icons/), via customLabel/customIcon.
class SocialLinkEntryType extends AbstractType
{
    ...
    {
        $builder
            // Same searchable icon-grid widget as customIcon below, restricted to NETWORKS and storing the bare key ("facebook") instead of an icon path - see IconPickerType's "icons"/"value_field" options
