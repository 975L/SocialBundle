...
class ShareButtonsSettingsCrudController extends AbstractCrudController
{
    ...
    // Redirects to editing the existing singleton instead of letting a second "share_buttons_settings"
    // Block be created - see SocialLinksCrudController::new() for why a duplicate row is a silent bug,
    // not a hard error
    ...
    public function configureCrud(Crud $crud): Crud
    {
        return $crud
            ...
            // Renders the style preview below the style <select> - see
            // share_buttons_style_preview_theme.html.twig and ShareButtonsStylePreviewType
    ...
    {
        return [
            // Virtual index columns summarizing "data" (see SocialLinksCrudController for why these
            // use a fake property name + setValue('') instead of a real, Doctrine-mapped one)
            ...
            // HiddenField, not Field/TextField: see SocialLinksCrudController for why a plain Field
            // gets silently rebuilt into an ArrayField/TextField that crashes against a JSON column
            HiddenField::new('data')
                ->setLabel(t('label.share_buttons_settings', [], 'social'))
                ->setFormType(ShareButtonsSettingsType::class)
                ->onlyOnForms(),

            // Live preview updated client-side (see assets/js/share-buttons-preview.js) as the style
            // <select> and the "networks" checkboxes above change, or are drag-reordered (see
            // assets/js/share-buttons-networks-sort.js). Field::setTemplatePath() is only honored on
            // index/detail pages, never on New/Edit (EasyAdmin's CrudFormType always builds a real Symfony form
            // field there instead) - ShareButtonsStylePreviewType + its form theme widget block render
            // the actual preview markup. "mapped" => false: unlike onlyOnIndex() fields above, this one
            // is onlyOnForms(), so EasyAdmin builds a real Symfony form field for it
            // (CrudFormType::buildForm()) - setValue('') alone only overrides EasyAdmin's own display
            // value, but Symfony's form data mapper still tries reading the (fake, non-existent)
            // property off Block directly, crashing with "Can't get a way to read the property".
            // Unmapping it skips that read entirely.
