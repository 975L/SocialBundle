...
`SocialGuidedProjectProvider` (implements ConfigBundle's `GuidedProjectProviderInterface`, auto-tagged like `MenuProviderInterface`) contributes three replayable exercises to the `/management` dashboard's "Guided projects" panel: **"Mettre les liens vers vos réseaux"** (one list for the whole site, rendered wherever the block is put), **"Régler les boutons de partage"** (which networks, in which order, and what they look like) and **"Afficher les avis Google de votre établissement"**. They run at 4010, 4020 and 4030, inside the 4000 block `GuidedProjectProviderInterface` reserves this bundle — that docblock states every bundle's, so the range is read there rather than recopied.
