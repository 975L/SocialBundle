...
- **Block silhouettes** via UiBundle's `BundleStylesheetManagementProviderInterface` — the bundle's kinds are drawn at thumbnail size in the back-office's visual block picker
...
While that key reads `false`, the "Boutons de partage" screen governs nothing and `MenuProvider` leaves it out — but the section doesn't fall silent about a feature the bundle ships: a **"Boutons de partage (désactivés)"** entry takes its place, sorted where the screen itself sits, and opens the `social-enable-share-buttons` config straight at its own edit form (through UiBundle's `ConfigEditUrlResolver`, falling back to the config list for a slug not yet loaded into the database). It is a link rather than a menu, naming no `target` so it stays inside the section, and it demands `site-role-admin` — the bar `ConfigCrudController` states on itself — where the settings screen behind the switch only asks for `site-role-editor`.
...
The whole feature hangs on one key, Ui's **`ui-enable-reviews`** (bool, `false` by default). Turned off, this bundle drops its own two halves of it: the "Connecter Google" link and the two Google guided projects — a site that shows no review has no reason to carry the screens explaining them. The import command keeps running, so reactivating it shows the reviews that came in meanwhile.
...
| Key | Filled by |
| --- | --- |
| `social-google-oauth-client-id` | the admin, from the Google Cloud console |
| `social-google-oauth-client-secret` | the admin — `sensitive`, so encrypted at rest by ConfigBundle's `VaultEncryptor` |
| `social-google-oauth-refresh-token` | the connection itself — `sensitive` |
| `social-google-business-account-id` | the connection itself |
| `social-google-business-location-id` | the connection itself |
...
`SameAsProvider` implements [c975L/UiBundle](https://github.com/975L/UiBundle)'s `SameAsProviderInterface`, so a page carrying a `contact_details` block publishes, in its `sameAs`, the Google listing and every social link this bundle already stores — nothing of it is retyped into the contact form.
