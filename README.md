...
The share buttons project is contributed **only while `social-enable-share-buttons` is on**, and the two Google ones **only while `ui-enable-reviews` is on** — the same condition `MenuProvider` applies to its own "Connecter Google" link, since with the feature off that screen isn't in the sidebar either and a parcours walking to an unreachable screen reads as a broken one.
