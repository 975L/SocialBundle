...
class SkillsTest extends TestCase
{
    ...
    private const string SLUG_PATTERN = '#^social-[a-z-]+$#';

    private const string NAME_PATTERN = '#^[a-z][a-z0-9]*(_[a-z0-9]+)+(\([^)]*\))?$#';

    private const string COMMAND_PATTERN = '/c975l:social:[a-z-:]+/';

    private const string COMPONENT_PATTERN = '/<twig:c975LSocial:([A-Za-z:]+)/';
    ...
    // The file a class name stands for, src/ being flat enough for the basename to be unique - a relative namespace ("Service\ShareButtonsService") is resolved as written, a bare name is looked up
    ...
    public function testEveryClassMemberQuotedExists(): void
    {
        foreach ($this->loadSkills() as $directory => $content) {
            foreach ($this->backtickedTokens($content, '#^(?!c975L\\\\|App\\\\)[A-Z][A-Za-z]*(\\\\[A-Z][A-Za-z]*)*(::[A-Za-z_]+(\(\))?)?$#') as $token) {
                ...
                // Only what this bundle could hold: a namespaced name is checked when its first segment is a real folder of src/ ("Service\ShareButtonsService"), a bare one when src/ holds a file of that name at all - Symfony's own "Assert\Email" or "FormBuilder::add()" are neither, and are no business of this repository
