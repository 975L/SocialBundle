...
class ShareButtonsSettingsCrudControllerTest extends TestCase
{
    ...
    // A plain Field/TextField gets silently rebuilt by EasyAdmin against a JSON column - HiddenField has no dedicated configurator, so setFormType() below fully takes over as intended (see the class-level comment on the controller)
    ...
    // "mapped" => false: this field has no matching Block property, so Symfony's form data mapper would otherwise crash trying to read/write it (see the class-level comment on the controller)
    ...
    // Redirects to editing the existing singleton instead of letting a second "share_buttons_settings" Block be created (see the class-level comment on the controller); the no-existing-block branch delegates to parent::new(), which needs full EasyAdmin/container wiring (event dispatcher, security voter, form factory...) disproportionate to this pure-unit suite, so it's left untested here
