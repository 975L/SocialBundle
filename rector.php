...
// The same sets a site gets from SymfonyMigrate.sh, deliberately. scaffold/ is listed next to the bundle's own code so any PHP added there is modernised here: the scaffold is copied as is into the applications, where that configuration runs over it, and a file rewritten downstream no longer matches the hash ScaffoldInstaller recorded, so the site is told forever it customized a file it never touched
